<script lang="ts">
  ...
  const filterTriggerContent = $derived(() => {
    if (filterType === "all")
      return `${m["library.all"]()} ${m["library.types_suffix"]()}`;
  ...
  const itemsPerPageTriggerContent = $derived(
    () => `${itemsPerPage} ${m["library.per_page_suffix"]()}`
  );
  ...
  const filteredMedia = $derived(() => {
    ...
    // Filter out audio types (audio is shown on /audio page instead)
    // The API returns all media types, but we only display images and videos here
    media = media.filter(
  ...
  function getMediaUrl(item: MediaItem): string {
    ...
    if (item.type === "image") return `/api/images/${item.id}`;
    return `/api/videos/${item.id}`;
  }
...
<main class="flex-1 p-6 overflow-auto">
  <div class="max-w-7xl mx-auto">
    ...
    >
      <!-- Stats -->
      <div
        class="flex items-center gap-4 text-sm text-muted-foreground flex-wrap"
      >
        ...
        <Select.Root type="single" bind:value={filterType}>
          ...
          <Select.Content>
            <Select.Item value="all"
              >{m["library.all"]()} {m["library.types_suffix"]()}</Select.Item
            >
        ...
        <Select.Root type="single" bind:value={sortBy}>
          ...
          <Select.Content>
            <Select.Item value="newest"
              >{m["library.newest"]()}
              {m["library.first_suffix"]()}</Select.Item
            >
        ...
        >
          ...
          <Select.Content>
            <Select.Item value="12"
              >12 {m["library.per_page_suffix"]()}</Select.Item
            >
            <Select.Item value="24"
              >24 {m["library.per_page_suffix"]()}</Select.Item
            >
            <Select.Item value="48"
              >48 {m["library.per_page_suffix"]()}</Select.Item
            >
          </Select.Content>
        </Select.Root>
      </div>
    ...
    {:else if filteredMedia().length === 0}
      <div class="text-center py-12">
        ...
        <Button onclick={() => goto("/newchat")}
          >{m["library.start_creating"]()}</Button
        >
      </div>
            ...
            <div class="cursor-default p-2.5">
              ...
              <div class="space-y-1.5 text-xs text-muted-foreground">
                {#if typedItem.chatModel}
                  <div>
                    {m["library.model_label"]()}:
                    <span class="font-medium">{typedItem.chatModel}</span>
                  </div>
                {/if}

                <div>
                  {m["library.size_label"]()}:
                  <span class="font-medium"
                    >{formatFileSize(typedItem.fileSize)}</span
                  >
                </div>
                <div>
                  {m["library.created_label"]()}:
                  <span class="font-medium"
          ...
          <div class="flex flex-col gap-2 text-sm">
            ...
            {#if image.chatModel}
              <div class="text-white/80">
                {m["library.model_label"]()}: {image.chatModel}
              </div>
            {/if}
            <div class="flex items-center gap-4 text-white/80 text-xs">
              <span
                >{m["library.size_label"]()}: {formatFileSize(
                  image.fileSize
                )}</span
              >
              <span
                >{m["library.created_label"]()}: {formatDate(
                  image.createdAt
                )}</span
              >
            </div>
