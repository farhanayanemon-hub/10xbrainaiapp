...
export class StripeService {
        ...
        static async createCreditCheckoutSession({
                ...
                try {
                        ...
                        const session = await stripe.checkout.sessions.create({
                                ...
                                mode: 'payment',
                                return_url: `${process.env.ORIGIN || ''}/settings/billing?credit_purchase=success`,
                                metadata: {
                                        userId,
                                        creditPlanId,
        ...
        private static async handleCreditPurchaseCompleted(session: Stripe.Checkout.Session): Promise<void> {
                const { CreditService } = await import('./credit-service.js');

                const userId = session.metadata?.userId;
                const creditPlanId = session.metadata?.creditPlanId;

                if (!userId || !creditPlanId) {
                        console.error('Missing userId or creditPlanId in credit purchase session metadata');
                        return;
                }
                ...
                try {
                        await CreditService.purchaseCredits(userId, creditPlanId, 'stripe', sessionId);
                        console.log(`Credit purchase completed: user=${userId}, plan=${creditPlanId}, session=${sessionId}`);
                } catch (error) {
